...
class CacheService {
  constructor() {
    ...
    this.maxStorageSize = 50 * 1024 * 1024; // 50MB max localStorage usage
    
    // Cache configuration for different data types
    this.cacheConfig = {
      // Static data - cache for longer
      parks: { ttl: 24 * 60 * 60 * 1000, storage: 'localStorage' }, // 24 hours
      ...
      blogPosts: { ttl: 30 * 60 * 1000, storage: 'localStorage' }, // 30 minutes
  ...
  enforceStorageLimit() {
    ...
    if (totalSize > this.maxStorageSize) {
      ...
      for (const entry of entries) {
        if (totalSize - removedSize <= this.maxStorageSize * 0.8) break;
