/**
 * One-off script: Send blog notification for the Arches post to all 20 users.
 * Usage: node send-blog-notification.js
 */
...
async function main() {
  ...
  // Fetch the Death Valley blog post
  const db = mongoose.connection.db;
  const post = await db.collection('blogposts').findOne({ slug: 'death-valley-national-park-the-complete-2026-visitor-guide' });
  ...
  // TEST MODE: Send only to Krishna first
