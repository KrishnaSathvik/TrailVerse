...
const VerifyEmailPage = () => {
  ...
  useEffect(() => {
    const verifyEmail = async () => {
      try {
        ...
        if (response.token && response.data) {
          ...
          setTimeout(() => {
            router.push('/explore?filter=national-parks');
          }, 2000);
        }
      } catch (error) {
        setStatus('error');
    ...
    verifyEmail();
  }, [token, router, setUserAfterVerification]);
      ...
      <div className="flex-1 flex items-center justify-center p-8" style={{ backgroundColor: 'var(--bg-primary)' }}>
        <div className="w-full max-w-md">
          ...
          >
            ...
            {status === 'success' && (
              <>
                ...
                <Link href="/explore?filter=national-parks">
          ...
          <p className="text-center text-xs mt-6" style={{ color: 'var(--text-tertiary)' }}>
            Having trouble?{' '}
            <Link href="/signup" className="hover:opacity-80 transition" style={{ color: 'var(--text-secondary)' }}>
              Sign up again
            </Link>
            {' '}or{' '}
