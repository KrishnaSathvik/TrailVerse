...
if (!OPENWEATHER_API_KEY) {
  console.warn('⚠️ NEXT_PUBLIC_OPENWEATHER_API_KEY is not set! Weather data will fall back to sample data.');
  console.warn('Environment check:', {
    NODE_ENV: process.env.NODE_ENV,
    hasKey: !!process.env.NEXT_PUBLIC_OPENWEATHER_API_KEY,
    keyLength: process.env.NEXT_PUBLIC_OPENWEATHER_API_KEY?.length || 0
  });
} else {
