...
import type { WeatherWidgetProps as _WeatherWidgetProps } from '../../types/weather';

const WeatherWidget: React.FC<_WeatherWidgetProps> = ({ latitude, longitude, parkName: _parkName }) => {
  ...
  const getDisplayWeather = () => {
    ...
    return {
      ...
      forecast: forecast?.list?.slice(0, 5).map((item: any, _index: any) => ({
  ...
  const getWeatherIconFromCode = (iconCode: any) => {
    ...
    >
      ...
      {displayWeather && (
        <div className="space-y-2">
          {displayWeather.forecast.map((day: any, _index: any) => {
